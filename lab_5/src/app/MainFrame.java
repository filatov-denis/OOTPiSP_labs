...
public final class MainFrame extends JFrame {
    ...
    private final XmlSerializer ser;
    private final DefaultListModel<Vehicle> model = new DefaultListModel<>();
    private final JList<Vehicle> list = new JList<>(model);

    public MainFrame(Registry reg, Processors processors) {
        super("Lab 5 — Plugins (XML <--> JSON)");
        this.reg = reg;
        this.processors = processors;
        this.ser = new XmlSerializer(reg);
        ...
        save.addActionListener(e -> withChooser(true, p -> Files.write(p, processors.applySave(ser.toBytes(toList())))));
        load.addActionListener(e -> withChooser(false, p -> {
            List<Vehicle> loaded = ser.fromBytes(processors.applyLoad(Files.readAllBytes(p)));
            model.clear();
            loaded.forEach(model::addElement);
        }));
        file.add(save); file.add(load);

        // Settings menu is built dynamically from loaded processor plugins.
        JMenu settings = new JMenu("Processors");
        for (ProcessorPlugin p : processors.all()) {
            JCheckBoxMenuItem cb = new JCheckBoxMenuItem(p.name(), processors.isEnabled(p.name()));
            cb.addActionListener(e -> processors.setEnabled(p.name(), cb.isSelected()));
            settings.add(cb);
        }
        if (settings.getItemCount() == 0) settings.add(new JMenuItem("(no plugins)")).setEnabled(false);

        bar.add(file); bar.add(addMenu); bar.add(act); bar.add(settings);
        setJMenuBar(bar);
    }
